...
- **新闻内嵌组件**：
  - **比赛比分卡**：队伍、系列比分、逐图比分与双方选手统计表（K-D / Swing / ADR / KAST / Rating 3.0），点击在扩展内打开比赛详情
  - **未来赛程卡**：即将进行的对阵（本地时间），点击跳转比赛详情
  - **Read More 引用卡**：相关文章链接，点击在扩展内打开
  - **文中战队**：从战队悬浮卡提取每支被提及战队的排名与五人名单，去重后集中展示（此前被渲染成零散碎片）
